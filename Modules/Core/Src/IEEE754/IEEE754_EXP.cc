...
_Check_return_ _FType __cdecl exp(_In_ _FType _X) { return __exp(_X); };

_EXTERN_C

// --------------------DOUBLE-PRECISION-------------------- //

static const unsigned long long __Expf64_table[]
{
...
_Check_return_ __Float64 __cdecl __IEEE754_EXPF64(_In_ __Float64 _X)
{
    ...
    sbits = __Expf64_table[idx + 1] + top; // This is only a valid scale when -1023*N < k < 1024*N.
