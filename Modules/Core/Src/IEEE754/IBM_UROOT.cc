...
static const double __IBM_inroot_table[128] =
{
...
_Check_return_ __Float64 __cdecl __IBM_SQRTF64(_In_ __Float64 _X)
{
    #if !defined(_USE_CUSTOM_FUNCTIONS) && ((defined _MSC_VER && _MSC_VER >= 1900) || defined(__EMSCRIPTEN__)) && !defined(_QUAD_PRECISION)
    return _CSTD sqrt(_X);
    #else

    /* Use generic implementation.  */

    // Coefficients of Taylor series.
    // inversesqrt(1 + x) = 1 - (1/2)*x + (3/8)*x^2 - (5/16)*x^3 + (35/128)*x^4 - (63/256)*x^5 + ... (((-1)^n*(2*n)!)/(4^n*(n!)^2))
    ...
    else
    {
        ...
        if (k < 0)
            return __Float64::FromBytes(S_NAN_DOUBLE); /* sqrt(-ve)=sNaN */
        return 0x1p-256 * __IBM_SQRTF64(x * 0x1p512);
    }

    #endif
}

// This is a simple routine for solving square root
// method: sqrt(x) = pow(x, 0.5) = exp(0.5 * ln(x))
_Check_return_ _FType __cdecl __CosmoCXX_Fast_SquareRoot(_In_ _FType _X)
{
    return __exp(0.5 * __ln(_X));
}
