...
_Check_return_ _FType __cdecl log(_In_ _FType _X) { return __log(_X); };
...
_Check_return_ _FType __cdecl __IEEE754_LOGF128_CF64(_In_ _FType _X)
{
    #if !defined(_USE_CUSTOM_FUNCTIONS) && ((defined _MSC_VER && _MSC_VER >= 1900) || defined(__EMSCRIPTEN__))
    return _CSTD log10(_X);
    #else
    ...
    if (((hx & 0x7FFFFFFFFFFFFFFF) | lx) == 0) { return _FType::FromBytes(CSE_NEGATIVE_INF); }
    if (hx >= 0x7FFF000000000000) { return _FType::FromBytes(CSE_POSITIVE_INF); }
    #else
    hx = x.Bytes;
    if ((hx & 0x7FFFFFFFFFFFFFFF) == 0) { return _FType::FromBytes(CSE_NEGATIVE_INF); }
    if (hx >= 0x7FF0000000000000) { return _FType::FromBytes(CSE_POSITIVE_INF); }
    #endif
    if (hx < 0) { return _FType::FromBytes(CSE_NAN); }
    if (x == 1.0L) { return 0.0L; }

    int e;
    x = LOG_FREXP(x, &e);

    auto neval = [](_FType x, const _FType* p, int n)
    {
        _FType y;
        p += n;
        y = *p;
    ...
    auto deval = [](_FType x, const _FType* p, int n)
    {
        _FType y;
        p += n;
        y = x.Val + (*p).Val;
