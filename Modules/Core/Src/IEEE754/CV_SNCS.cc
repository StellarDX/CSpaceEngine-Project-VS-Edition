...
static const float64 __CV_SinCos_Tab[]
(
...
_Check_return_ float64 __cdecl __CV_SIN_ANGLES(_In_ float64 _X)
{
	...
	size_t TABSIZE = 64;
	float64 k1 = TABSIZE / 360.; // Base on angle
	static const float64 k2 = (2 * CSE_PI) / TABSIZE;
...
_Check_return_ float64 __cdecl __CV_TAN_ANGLES(_In_ float64 _X)
{
	...
	#else
	return _CSE __sin(_X) / _CSE __cos(_X);
	#endif
}
