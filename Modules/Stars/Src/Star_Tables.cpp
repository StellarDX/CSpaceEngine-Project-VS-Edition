...
_STD map<float64, _STD pair<SPECSTR, float64>> _Adopt_BC_Teff_Calib_For_Giants() // II, III, IV
{
	...
	_Par.emplace(_RGB_TABLE_TEFF_LOW_LIMIT, pair<SPECSTR, float64>("M7"  , -3.06)); // Using data from Mira Cet
	return _Par;
}
...
{
	if (_Teff <= _RGB_TABLE_TEFF_LOW_LIMIT)
	{
		_Param->first = _RAND _Giant_Params.begin()->second.first;
		_Param->second = _RAND _Giant_Params.begin()->second.second;
		return;
	}
	auto it = _RAND _Giant_Params.rbegin();
	auto end = _RAND _Giant_Params.rend();
